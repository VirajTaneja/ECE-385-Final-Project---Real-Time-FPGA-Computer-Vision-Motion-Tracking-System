...
u8 SendBufPtr[2];
u8 RecvBufPtr[2];
unsigned int Byte_count = 2; //not too sure about this
//write val via SPI
...
Status = XSpi_Transfer(&SpiInstance, SendBufPtr, NULL, Byte_count);
if (Status != XST_SUCCESS)
{
xil_printf ("printing error");
}
...
u8 SendBufPtr[nbytes + 1];
u8 RecvBufPtr[nbytes + 1];
unsigned int Byte_count = nbytes + 1;


for (unsigned int iterator = 0; iterator < nbytes; iterator++){

SendBufPtr[iterator + 1] = data[iterator];
...
//BYTE value_recieved = RecvBufPtr[1]; //idk which bit
...
//else{
//SendBufPtr[iterator] = 0x00;
//}
}

Status = XSpi_Transfer(&SpiInstance, SendBufPtr, RecvBufPtr, nbytes + 1);
if (Status != XST_SUCCESS)
{
xil_printf ("printing error");
}

for (unsigned int iterator2 = 0; iterator2 < nbytes; iterator2++){
data[iterator2] = RecvBufPtr[iterator2 + 1];
}
